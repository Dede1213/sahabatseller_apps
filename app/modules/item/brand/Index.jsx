  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        const response = await fetchData(`${API_HOST}/item-brand/store?page=1&limit=100${keyword}${paramOrder}`,
          {
            headers: {
      ...
      <View className="w-full h-full justify-center px-4 bg-primary">
        ...
        {user?.is_head_office == 'YA' && (
          <TouchableOpacity style={styles.circleButton} onPress={() =>
            navigation.navigate('Item', { screen: 'ItemBrandCreateStack' })}>
            <View testID="btn003">
