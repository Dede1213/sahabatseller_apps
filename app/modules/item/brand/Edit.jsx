...
const Edit = () => {
    ...
    useEffect(() => {
        const getData = async (id) => {
            try {
                const response = await fetchData(`${API_HOST}/item-brand/${id}`,
                    {
                        headers: {
    ...
    const submitAction = async (form) => {
        try {
            ...
            const response = await fetchData(`${API_HOST}/item-brand/update`, {
                method: 'put',
                headers: {
                    'X-access-token': user.token,
                    'Content-Type': 'application/json',
            ...
            if (response.code) {
                if (response.code === 200) {
                    showAlert('Notifikasi', 'Merek produk berhasil diubah.');
                }
            } else {
                Alert.alert(response.message)
            }
    ...
    const handleDelete = async () => {
        try {
            setIsSubmitting(true)
            const response = await fetchData(`${API_HOST}/item-brand/delete/${id}`, {
                method: 'delete',
                headers: {
                    'X-access-token': user.token,
                    'Content-Type': 'application/json',
            ...
            if (response.code) {
                if (response.code === 200) {
                    showAlert('Notifikasi', 'Merek produk berhasil dihapus.');
                }
            } else {
                Alert.alert(response.message)
            }
                    ...
                    {({ handleChange, handleBlur, handleSubmit, values, errors, touched }) => (
                        <View>
                            ...
                            <View>
                                ...
                                <CustomButton
                                    title="Hapus Brand"
                                    handlePress={() => showAlertConfirm('Konfirmasi', 'Apakah Anda yakin ingin menghapus brand ini?')}
