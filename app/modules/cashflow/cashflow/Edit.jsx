...
const Edit = () => {
  ...
  useEffect(() => {
    const getData = async (id) => {
      try {
        const response = await fetchData(`${API_HOST}/cash-flow/${id}`,
          {
            headers: {
        ...
        if (response.code) {
          if (response.code === 200) {
            setInitialValues({
              category: (response.data.category_id).toString() || '',
              type: response.data.types || '',
              amount: (response.data.amount).toString() || '',
              note: response.data.note || '',
              transaction_date: new Date(response.data.transaction_date) || new Date(),
            });
          }
        } else {
  ...
  const submitAction = async (form) => {
    try {
      ...
      const response = await fetchData(`${API_HOST}/cash-flow/update`, {
        method: 'put',
        headers: {
          'X-access-token': user.token,
          'Content-Type': 'application/json',
      ...
      if (response.code) {
        if (response.code === 200) {
          showAlert('Notifikasi', 'Cash flow berhasil diubah.');
        }
      } else {
        Alert.alert(response.message)
      }
  ...
  const handleDelete = async () => {
    try {
      setIsSubmitting(true)
      const response = await fetchData(`${API_HOST}/cash-flow/delete/${id}`, {
        method: 'delete',
        headers: {
          'X-access-token': user.token,
          'Content-Type': 'application/json',
      ...
      if (response.code) {
        if (response.code === 200) {
          showAlert('Notifikasi', 'Cash flow berhasil dihapus.');
        }
      } else {
        Alert.alert(response.message)
      }
          ...
          {({ handleChange, handleBlur, handleSubmit, values, errors, touched, setFieldValue }) => (
            <View>
              ...
              <CustomButton
                ...
                handlePress={() => showAlertConfirm('Konfirmasi', 'Apakah Anda yakin ingin menghapus cash flow ini?')}
