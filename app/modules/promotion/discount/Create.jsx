...
import { FormatDateForQuery, FormatTimeForQuery } from '../../../../lib/globalFunction'
...
const Create = () => {
  ...
  const submitAction = async (form) => {
    ...
    try {
      ...
      const response = await fetchData(`${API_HOST}/discount/create`, {
        method: 'post',
        headers: {
      ...
      if (response.code) {
        if (response.code === 200) {
          showAlert('Notifikasi', 'Diskon berhasil ditambah.');
        }
      } else {
                    ...
                    <View className="w-full">
                      <TouchableOpacity onPress={() => setShowTimePicker(true)}>
                        <View className={`border-2 border-gray-200 w-full px-2 rounded-lg focus:border-secondary items-center flex-row`}>
                          <View className="flex-1 py-3">
                            <Text className="text-gray-100 font-poppinsSemiBold text-m">
                              {values.start_time ? (values.start_time).toLocaleTimeString() : (new Date()).toLocaleTimeString()}
                            </Text>
                          </View>
