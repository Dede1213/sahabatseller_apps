...
const Variant = () => {
  ...
  const handleModal = async (id, param) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetchData(`${API_HOST}/history/${param}/${id}?page=1&limit=50&order_by=id desc`,
        {
          headers: {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        const response = await fetchData(`${API_HOST}/item-bundling/variant?item_id=${id}&page=1&limit=100${keyword}${paramOrder}`,
          {
            headers: {
              'X-access-token': user.token,
              'Content-Type': 'application/json',
              ...
              <View className="flex-row">
                ...
                <View className="w-[75%] ml-1">
                  ...
                  >
                    <View className="">
                      <Text className="text-sm font-PoppinsSemiBold text-blue-200 ml-1" testID='btn-001'>
                        {(item.name).length > 31 ? (CapitalizeEachWord(item.name)).substring(0, 32) + '...' : (CapitalizeEachWord(item.name))}
                      </Text>
                    </View>
                  ...
                  <View className="flex-row mt-1.5 ml-1">
                    ...
                    <View className="mr-2">
                      <TouchableOpacity
                        onPress={() => { handleModal(item.id, 'hpp') }}
                        activeOpacity={0.5}
                        className='rounded-[3px] min-h-[20px] min-w-[60px] justify-center bg-gray-500 px-0.5'
                      >
                        <View className="flex-row">
                    ...
                    <ScrollView>
                      {dataModal?.map((item, index) => (
                        <View key={index} style={styles.tableRow}>
                          ...
                          <View className="w-[70px]">
                            <Text style={styles.tableCell} className="text-right">{modalActive == 'hpp' ? FormatAmount(item.old_hpp) : modalActive == 'price' ? FormatAmount(item.old_price) : FormatAmount(item.old_stock)}</Text>
                          </View>
                          <View className="w-[70px] mr-4">
                            <Text style={styles.tableCell} className="text-right">{modalActive == 'hpp' ? FormatAmount(item.new_hpp) : modalActive == 'price' ? FormatAmount(item.new_price) : FormatAmount(item.new_stock)}</Text>
                          </View>
