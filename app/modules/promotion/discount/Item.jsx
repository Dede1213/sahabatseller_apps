  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        const response = await fetchData(`${API_HOST}/item-bundling/item?item_id=${id}&page=1&limit=100${keyword}${paramOrder}`,
          {
            headers: {
              'X-access-token': user.token,
              'Content-Type': 'application/json',
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      navigation.navigate('Item', {
                        screen: 'ItemBundlingVariantStack',
                        params: { id: item.id }
                      });
                    }}
                  >
                    <View className="">
                      <Text className="text-sm font-PoppinsSemiBold text-blue-200 ml-1" testID='btn-001'>
                        {(item.name).length > 51 ? (CapitalizeEachWord(item.name)).substring(0, 50) + '...' : (CapitalizeEachWord(item.name))}
                      </Text>
                    </View>
                  </TouchableOpacity>
                </View>
