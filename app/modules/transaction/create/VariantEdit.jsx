...
const VariantEdit = () => {
    ...
    const { user, setRefreshTrigger, setTransaction, transaction } = useGlobalContext()
    ...
    const handleDiscount = async (value, resetForm) => {
        if (initialValues.price > 0) {
            ...
            if (isPercent) {
                discount_item_amount = Math.round((price * discount) / 100);
            } else {
                discount_item_amount = Math.round(discount);
            }
